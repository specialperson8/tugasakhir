...
<div class="sub-banner" style="background:url({{ URL::asset('upload/'.getcong('page_bg_image')) }}) no-repeat center top;">
  <div class="overlay">
    <div class="container">
      <h1>{{$total_res}} Pencarian Untuk {{$keyword}}</h1>
    </div>
  </div>
</div>
        ...
        <div data-wow-delay="0.{{$i}}s" class="strip_list wow fadeIn animated" style="visibility: visible; animation-delay: 0s; animation-name: fadeIn;">
          <div class="row">
              ...
              <div class="desc">
                ...
                <div class="rating">
                  @for($x = 0; $x < 5; $x++) @if($x < $restaurant->review_avg)
                    ...
                    @endfor
                    (<small><a href="{{URL::to('restaurants/'.$restaurant->restaurant_slug)}}">Lihat {{\App\Review::getTotalReview($restaurant->id)}} Penilaian</a></small>)
                </div>
            ...
            <div class="col-md-3 col-sm-12">
              <div class="go_to">
                <div> <a class="btn_1" href="{{URL::to('restaurants/menu/'.$restaurant->restaurant_slug)}}">Lihat Menu</a> </div>
              </div>
            </div>
          </div>
        </div>
