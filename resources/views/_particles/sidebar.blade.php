<div class="col-md-3 col-sm-5 col-xs-12 side-bar">
    ...
    <div id="cart_box">
        <h3>Pesanan Anda<i class="fa fa-shopping-cart pull-right"></i></h3>
        ...
        <table class="table table_summary">
            <tbody>
                <tr>
                    <td><a href="{{ URL::to('delete_item/' . $cart_item->id) }}" class="remove_item"><i class="fa fa-minus-circle"></i></a> <strong>{{ $cart_item->quantity }}x</strong>
                        {{ $cart_item->item_name }}
                    </td>
                    <td><strong class="pull-right">{{ getcong('currency_symbol') }}{{ $cart_item->item_price }}.000</strong>
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="table table_summary">
            <tbody>
                <tr>
                    <td class="total"> TOTAL <span class="pull-right">{{ getcong('currency_symbol') }}{{ $price = DB::table('cart')->where('user_id', Auth::id())->sum('item_price') }}.000</span>
                    </td>
                </tr>
            </tbody>
        </table>
